from collections import defaultdict


def exists_symbol_nearby(start_pos: tuple[int, int],
                         len_number: int,
                         map: dict[tuple[int, int], str],):
    for i in range(max(start_pos[0] - 1, 0), start_pos[0] + 2):
        for j in range(max(start_pos[1] - 1, 0), start_pos[1] + len_number + 1):
            if result := map.get((i, j)):
                if result == '*':
                    return (i, j)
    return False


def main(file_path: str):
    cand_gears = defaultdict(list)
    with open(file_path) as file:
        map = {}
        for i, line in enumerate(file):
            for j, char in enumerate(line.strip() + '.'):
                map[i, j] = char
        current_number = ''
        for key, value in map.items():
            if str.isnumeric(value):
                if current_number == '':
                    start_pos = key
                current_number += value
            else:
                if current_number != '':
                    final_j = key[1]
                    number_lenght = final_j - start_pos[1]
                    if (symbol_pos := exists_symbol_nearby(start_pos, number_lenght, map)):
                        cand_gears[symbol_pos].append(current_number)
                    current_number = ''

    gears = [value for _, value in cand_gears.items() if len(value) == 2]
    sum = 0
    for gear in gears:
        code = 1
        for part in gear:
            code *= int(part)
        sum += code
    return sum


if __name__ == '__main__':
    print(main('files/input.txt'))
